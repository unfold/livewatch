...
function livewatch(command, sourcePaths, targetPaths, debug) {
    ...
    function executeCommand() {
        ...
        try {
            exec(command, function(err, stdout, stderr) {
                ...
                stderr.length && console.error(indent(stderr.toString()))
                debug && stdout.length && console.log(indent(stdout.toString()))

                flushChanges()
            })
        } catch (e) {
            console.error('Error executing command: %s', e.message)
        }
    }

    function flushChanges() {
        for (var targetPath in changedTargets) {
            ...
            delete changedTargets[targetPath]
        }
    }
    ...
    watchr.watch({
        ...
        listeners: {
            ...
            change: function(type, targetPath) {
                if (!changedTargets[targetPath]) {
                    changedTargets[targetPath] = type
                }
            }
        }
    })
